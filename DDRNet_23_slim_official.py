...
class DualResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        comparison['layer1'] = self.relu(x)

        layers.append(x)

        x = self.layer2(self.relu(x))

        comparison['layer2'] = self.relu(x)
        ...
        comparison['layer3_'] = x_

        x = x + self.down3(self.relu(x_))

        comparison['down3'] = self.relu(x)
        ...
        comparison['copression3'] = self.relu(x_)
        ...
        comparison['layer4_'] = x_


        x = x + self.down4(self.relu(x_))

        comparison['down4'] = self.relu(x)
        ...
        comparison['compression4'] = self.relu(x_)

        x_ = self.layer5_(self.relu(x_))

        comparison['layer5_'] = x_

        comparison['layer5'] = self.layer5(self.relu(x))

        comparison['spp0'] = self.spp(self.layer5(self.relu(x)))
...
def DualResNet_imagenet_off(pretrained=False):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=19, planes=32, spp_planes=128, head_planes=64, augment=False)
    if pretrained:
        pretrained_state = torch.load('DDRNet23s_imagenet.pth', map_location='cpu') 
...
def get_seg_model(**kwargs):
    ...
    model = DualResNet_imagenet_off(pretrained=True)
    return model
